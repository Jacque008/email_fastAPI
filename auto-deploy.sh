...
if [ "$1" = "prod" ]
then
    SERVICE_NAME="classify-emails-fastapi"
    ...
    --cpu 1 \
    --timeout 3600 \
    --set-env-vars ENV_MODE=production,DB_USER=drp,DB_NAME=drp-production,INSTANCE_CONNECTION_NAME=drp-system:europe-west4:drp \
    --set-secrets DB_PASSWORD=DRP_DB_PASSWORD:latest,GROQ_API_KEY=GROQ_API_KEY:latest,SECRET_KEY=FLASK_SECRET_KEY:latest,SECRET_KEY_FASTAPI=SECRET_KEY_FASTAPI:latest,JWT_SECRET_FASTAPI=JWT_SECRET_FASTAPI:latest,GOOGLE_CLIENT_ID=GOOGLE_CLIENT_ID:latest,GOOGLE_CLIENT_SECRET=GOOGLE_CLIENT_SECRET:latest \
