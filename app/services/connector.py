...
import pandas as pd
from .utils import fetchFromDB, check_eq, pick_first, check_full_parts_match, list_deduplicate
...
class Connector(Processor):
    def __init__(self) -> None:
        super().__init__()
        self.errandConnectQuery = self.queries['errandConnect'].iloc[0]
    ...
    def fetch_and_format_errand(self, condition: str) -> Optional[pd.DataFrame]:
        query = self.errandConnectQuery.format(CONDITION=condition)
    ...
    def errand_row_filter(self, row, email_sender, email_receiver):
    ...
    def filter_candidate_errand(self, email_row: pd.Series, errand: pd.DataFrame) -> pd.DataFrame:
        ...
        mask = cand.apply(lambda row: self.errand_row_filter(row, email_sender, email_receiver), axis=1)
        # if not mask.any():
    ...
    def find_match_for_single_email(self, email_row: pd.Series, errand: pd.DataFrame) -> pd.Series:  
        matched_errand, connected_col, note = self.match_by_reference(email_row, errand)
        if matched_errand is not None:   
            return self.fill_back_result(email_row, matched_errand, connected_col, note)
        
        cand = self.filter_candidate_errand(email_row, errand)
        if not cand.empty:
            matched_errand, connected_col, note = self.match_by_number(email_row, cand)
            if matched_errand is None:
                matched_errand, connected_col, note = self.match_by_name(email_row, cand)
            if matched_errand is not None:
                return self.fill_back_result(email_row, matched_errand, connected_col, note)
        
        return {"errand_matched": False}
        
    def match_by_reference(self, email_row: pd.Series, errand: pd.DataFrame) -> Tuple[Optional[pd.Series], Optional[str], Optional[str]]:
    ...
    def match_by_number(self, email_row: pd.Series, cand: pd.DataFrame) -> Tuple[Optional[pd.Series], Optional[str], Optional[str]]:
        # print("match_by_number: ", email_row['reference'], "cand:", cand.shape)
        ...
        for col in ['insuranceNumber', 'damageNumber']:
            ...
            if pd.isna(email_settle) and pd.isna(email_total):
                ...
                if hit is not None:
                    # print("match_by_number-damageNumber",hit, col, 'Unreliable')
            ...
            if pd.notna(email_settle) and 'settlementAmount' in possible.columns:
                ...
                if hit is not None:
                    # print("match_by_number-settlementAmount",hit, col, 'Reliable')
            ...
            if pd.notna(email_total) and 'totalAmount' in possible.columns:
                ...
                if hit is not None:
                    # print("match_by_number-totalAmount",hit, col, 'Reliable')
                    return hit, col, 'Reliable'
        # print("match_by_number-final", None, None, None)
        return None, None, None

    def match_by_name(self, email_row: pd.Series, cand: pd.DataFrame) -> Tuple[Optional[pd.Series], Optional[str], Optional[str]]:
    ...
    def fill_back_result(self, email_row: pd.Series, matched_errand: pd.Series, connected_col: str, note: str) -> Dict[str, Any]:
