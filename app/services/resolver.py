...
from .utils import (fetchFromDB, base_match, lower_and_split, parse_email_address,
extract_first_address, expand_matching_clinic, get_staffAnimal)
...
class ReceiverResolver(BaseService):
    ...
    def set_receiver_clinic(self, exploded: pd.DataFrame) -> pd.DataFrame:
        ...
        if mask.any():
            exploded.loc[mask, 'clinicName'] = (exploded.loc[mask].apply(
                lambda row: expand_matching_clinic(row['parsedTo'], self.clinic_keywords), axis=1)
            )
        
        # Set originRecipient and sendTo
        ...
        mask_clinic = (exploded['originRecipient'].notna()) & (exploded['source'] != 'Clinic')
    ...
    def _special_email_add(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        refDB = fetchFromDB(self.errandInfoQuery.format(COND=f"ic.reference IN ({ref_list})"))
    ...
    def set_receiver_fb(self, exploded: pd.DataFrame) -> pd.DataFrame:
        ...
        if mask_clinic_to_ic.any():
            exploded.loc[mask_clinic_to_ic, 'originRecipient'] = (
                exploded.loc[mask_clinic_to_ic, 'originRecipient'].replace(fb_name_mapping)
            )
    ...
    def set_receiver_drp(self, exploded: pd.DataFrame) -> pd.DataFrame:
        ...
        if mask.any():
            exploded.loc[mask, ['originRecipient', 'sendTo']] = ['DRP', 'DRP']
    ...
    def set_receiver_finance(self, exploded: pd.DataFrame) -> pd.DataFrame:
        ...
        if mask.any():
            exploded.loc[mask, 'originRecipient'] = (exploded.loc[mask, 'parsedTo'].apply(
    ...
    def _aggregate(self, filtered_df: pd.DataFrame) -> pd.DataFrame:
        ...
        subset_cols = ['sendTo', 'originRecipient', 'reference', 'errandId', 'category']
        agg_dict = {}
        for col in subset_cols:
            if col == 'errandId':
...
class StaffResolver(BaseService):
    ...
    def detect_staff_animals(self, df: pd.DataFrame) -> pd.DataFrame:
        needed = ['id','category','recipient','animalName','ownerName','subject','origin','email','isStaffAnimal']
        work = df[needed].copy()

        work['recipient']  = self._clean_staff_text(work['recipient'])
        work['recipient']  = self._normalize_staff_halsinge(work['recipient'])
        ...
        work['clinic_key'] = self._normalize_name(work['recipient'])
...
class AddressResolver(BaseService):
    ...
    def detect_forward_address(self, source: str, recipient: str, user_id: Optional[int] = None) -> str:
        ...
        result = (self._resolve_clinic_forward(source, recipient)
                     ._resolve_fb_forward(source, recipient)
                     .resolve_admin_details(user_id)
                     .result)
        
        return result['forwardAddress']
    
    def _resolve_clinic_forward(self, source: str, recipient: str):
        """Resolve forwarding for clinic emails"""
        if self.result['forwardAddress']:  
            return self
        
        if source == 'Clinic':
            forward_addr = self.fb_forw_add.get(recipient, "")
            if forward_addr:
                self.result.update({
    ...
    def _resolve_fb_forward(self, source: str, recipient: str):
        """Resolve forwarding for insurance company emails"""
        if self.result['forwardAddress']:  
            return self
        
        if source == 'Insurance_Company':
            clinic_match = self.clinic_forw_add[self.clinic_forw_add['clinicName'] == recipient]
