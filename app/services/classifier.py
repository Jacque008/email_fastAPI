...
class Classifier(BaseService):    
    def __init__(self):
        super().__init__()
        self.category_reg_list = pd.read_csv(f"{self.folder}/category_reg.csv")
        self.category_list = self.category_reg_list['category'].unique().tolist()
        self.category_patterns = {category: re.compile('|'.join(f"(?:{p})" for p in regs.dropna() if str(p)), re.IGNORECASE)
    ...
    def identify_first_info(self, infoDate, subject: str) -> str:
        ...
        isFirst = fetchFromDB(self.infoQuery.format(CONDITION=condition))
    ...
    def process_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if info_mask.any():
            df.loc[info_mask, 'note'] = df.loc[info_mask, ['date', 'subject']].apply(lambda row: identify_first_info(row['date'], row['subject']), axis=1)
        return df
    ...
    def get_ic_ref(self, errandId: int) -> str:
        errandInfo = fetchFromDB(self.errandInfoQuery.format(COND=f"er.id = {errandId}"))
    ...
    def process_icReference(self, df: pd.DataFrame) -> pd.DataFrame:
        mask_ic_ref = (df['insuranceCaseRef'].isna() & (df['errandId'].apply(len) == 1))
        if mask_ic_ref.any():
            df.loc[mask_ic_ref, 'insuranceCaseRef'] = (df.loc[mask_ic_ref, 'errandId'].apply(lambda x: get_ic_ref(x[0])))
        return df
    ...
    def categorize_emails(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for category in remaining_categories:
            if category in self.category_patterns:
                ...
                mask = unassigned_mask & df['email'].str.contains(pattern, na=False, regex=True)
     ...
     # distingish DR and SÄ
       # for FB
        ...
        is_dr_insurance = (df['paymentOption'].notna()) | \
                          ...
                          (df['errandId'].apply(lambda x: len(x) > 0))
       ...
       # further correcting
        has_errand = df['errandId'].apply(lambda x: len(x) > 0)
    ...
    def statistic(self, df):
        ...
        print(f"all_records: {all}, auto_categorized: {auto}, {auto/all*100:.2f}%, manual: {manu}, {manu/all*100:.2f}%, connected_to_errands: {connected}, {connected/all*100:.2f}%, single_errandId: {singleConnect}, {singleConnect/all*100:.2f}%, reliable_connection: {reliable}, {reliable/all*100:.2f}%")

        categoryList = [item for item in self.category.category_list if item not in ['Complement', 'Complement_Reply']]
        ...
        for category in categoryList:
            mask_connect = (df['errandId'].apply(len)>0)
            mask_unconnect = (df['errandId'].apply(len)==0)
