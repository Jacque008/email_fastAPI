...
@router.get("/forward")
async def forward_page(request: Request, user=Depends(get_current_active_user)):
...
async def forwarding_api(
    ...
    try:
        ...
        if isinstance(forwarding_data, dict):
            emailId = forwarding_data.get("emailId")
            if emailId is None:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Missing required field 'emailId'"
                )
        ...
        # Handle batch requests
            ...
            for i, data in enumerate(forwarding_data):
                ...
                if emailId is None:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail=f"Missing required field 'emailId' in item {i}"
                    )
